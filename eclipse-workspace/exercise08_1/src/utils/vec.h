...
struct Vec3
{
	...
	float normalize();
	/**
	 * Limits each vector component to the range (0,1).
	 */
	void clamp();
...
inline void Vec3::clamp()
{
	...
	{
		if (this->operator [](d) < 0.0f)
			this->operator [](d) = 0.0f;
		if (this->operator [](d) > 1.0f)
			this->operator [](d) = 1.0f;
	}
}
