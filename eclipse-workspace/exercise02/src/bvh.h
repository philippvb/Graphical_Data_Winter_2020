...
struct BVH
{
	 ...
	 * node.
	 * @param box Describes the volume to be split.
	 */
	void findSplitPlane(unsigned int* dimension, float* position,
			const AABB &box);
	 ...
	 * processed.
	 */
	void buildBVH(int nodeIndex, int triIndex, int numTris, int depth);
